...
void *handle_client(void *arg) {
    ...
    while (1) {
        ...
        int net_block_size;
        int read_bytes = recv(client_socket, &net_block_size, sizeof(net_block_size), 0);
        if (read_bytes <= 0) {
        ...
        while (bytes_remaining > 0) {
            int chunk_size = bytes_remaining > BUFFER_SIZE ? BUFFER_SIZE : bytes_remaining;
    ...
    // Registrar en el log
    FILE *log = fopen("/home/Desktop/TEC/Operativos/Tarea2/CE4304-Sistemas-Operativos/Tarea_1/src/imageserver.log", "a+");
    if (log) {
        time_t now = time(NULL);
        fprintf(log, "Imagen recibida: %d bytes - %s\n", total_received, ctime(&now));
        fclose(log);
    }
